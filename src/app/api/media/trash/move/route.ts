...
import { moveMediaToTrash } from "@/lib/server/mediaService"
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Move the media to trash
    const result = await moveMediaToTrash(mediaId, session.uid)
    ...
    return NextResponse.json(
      { error: "Failed to move media to trash", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    )
  }
}
