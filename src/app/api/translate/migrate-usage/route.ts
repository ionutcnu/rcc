...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (!success) {
        console.error("recordTranslationUsage returned false")
        return NextResponse.json({ error: "Failed to record translation usage" }, { status: 500 })
      }
    } catch (recordError) {
      console.error("Error in recordTranslationUsage:", recordError)
      return NextResponse.json(
        {
          error: "Failed to record translation usage",
          details: recordError instanceof Error ? recordError.message : String(recordError),
        },
        { status: 500 },
      )
    }
